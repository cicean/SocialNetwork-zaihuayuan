...
import com.gzqining.inthegarden.dialog.getGiftDialog;
...
public final class RongCloudEvent implements RongIM.OnReceiveMessageListener, RongIM.OnSendMessageListener, RongIM.GetUserInfoProvider, RongIM.GetFriendsProvider, RongIM.GetGroupInfoProvider, RongIM.ConversationBehaviorListener, RongIM.ConnectionStatusListener, RongIM.LocationProvider {
	...
	public RongIMClient.UserInfo getUserInfo(String userId) {
		...
		// return new RongIMClient.UserInfo(userId, "张三",
		// "file:///storage/sdcard0/share.jpg");//测试本地图片
	...
	public boolean onClickMessage(Context context, RongIMClient.Message message) {
		...
		if (message.getContent() instanceof LocationMessage) {
			...
			String json = gson.toJson(message.getContent());
			try {
				JSONObject jo = new JSONObject(json);
				String giftId = jo.getString("extra");
				// String giftId = new JSONObject(extra).getString("giftId");
				final getGiftDialog dialog = new getGiftDialog(context, R.style.MyDialog, giftId);
				dialog.show();
			} catch (Exception e) {
				e.printStackTrace();
			}
