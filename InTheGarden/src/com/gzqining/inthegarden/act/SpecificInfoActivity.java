...
public class SpecificInfoActivity extends BaseActivity implements OnClickListener, OnSaveListener, OnWheelChangedListener {
	...
	public void onClick(View v) {
		...
		case R.id.finish_btn:
			editUserData();
			// Intent intent = new
			// Intent(SpecificInfoActivity.this,LoginActivity.class);
	...
	public void taskCallBack(String jsonObject, int type) {
		...
		/*
		 * case Constans.REQ_UserInfo: //Toast.makeText(this,
		 * back.Getoauth_token(), Toast.LENGTH_LONG).show(); UserInfoBack back =
		 * (UserInfoBack) ReqJsonUtil.changeToObject(jsonObject,
		 * UserInfoBack.class); if(back.getCode().equals("00000")){
		 * if(back.getSex().equals("2")){ sex_tv.setText("女"); }else{
		 * sex_tv.setText("男"); }
		 * 
		 * @SuppressWarnings("rawtypes") Map<String ,String> map=new
		 * HashMap<String ,String>(); Map<String ,String[]> map2=new
		 * HashMap<String ,String[]>(); try {
		 * 
		 * JSONObject jsonObjectn = new JSONObject(jsonObject.toString());
		 * JSONArray jsonArray = jsonObjectn.getJSONArray("user_profile"); //
		 * StringBuffer sb = new StringBuffer(); for(int i = 2;i <
		 * jsonArray.length();i++){ JSONObject json = (JSONObject)
		 * jsonArray.opt(i); Career career = (Career)
		 * ReqJsonUtil.changeToObject(json.toString(), Career.class); // String
		 * field_id = json.getString("field_id"); // String field_key =
		 * json.getString("field_key"); // String field_name =
		 * json.getString("field_name"); // String form_type =
		 * json.getString("form_type"); // JSONArray form_default_value =
		 * json.getJSONArray("form_default_value"); // String field_data =
		 * json.getString("field_data");
		 * map.put(career.getField_name(),career.getField_data());
		 * map2.put(career.getField_key(),career.getForm_default_value()); }
		 * String[] career = map2.get("career"); category_money =
		 * map2.get("income"); category_height = map2.get("height");
		 * category_weight = map2.get("weight"); // JSONObject jsonObjecCareer =
		 * new JSONObject(career); // JSONArray career_arr =
		 * jsonObjecCareer.getJSONArray("career"); //
		 * System.out.println((JSONArray) map2.get("性别")); // // for(int i =
		 * 0;i<((JSONArray) map2.get("性别")).length();i++){ // // } // sex_dialog
		 * = (List<String>) map2.get("性别"); String date =
		 * getStrTime(map.get("生日")); birthday_tv.setText(date);
		 * address_tv.setText(map.get("出生地")); marry_tv.setText(map.get("婚恋"));
		 * height_tv.setText(map.get("身高")); weight_tv.setText(map.get("体重"));
		 * xueli_tv.setText(map.get("学历")); drink_tv.setText(map.get("饮酒"));
		 * occupation_tv.setText(map.get("职业"));
		 * language_tv.setText(map.get("语言"));
		 * income_tv.setText(map.get("年收入")); smoke_tv.setText(map.get("吸烟"));
		 * faith_tv.setText(map.get("信仰")); } catch (JSONException e) {
		 * e.printStackTrace(); } }else{ Toast.makeText(this, back.getMessage(),
		...
		case Constans.REQ_editUserData:
			...
			if (cb.getCode() == 00000) {
				...
				Toast.makeText(SpecificInfoActivity.this, cb.getMessage(), Toast.LENGTH_LONG).show();
